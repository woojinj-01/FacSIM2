...
def rank_move_box_region(net_type='global'):
    
    fig_path = f"./fig/rank_move_box_region_{net_type}.pdf"
    ...
    _rank_move_box_region(network_dict["Biology"], ax[0])
    _rank_move_box_region(network_dict["Computer Science"], ax[1])
    _rank_move_box_region(network_dict["Physics"], ax[2])

    # x_label = "Relative Movement of Faculty (Normalized)"
    y_label = "Relative Rank Move"
    ...
    handles = [Patch(facecolor=network_dict["Biology"].color, label="Biology", alpha=param_alpha, edgecolor='black', linewidth=3),
               Patch(facecolor=network_dict["Computer Science"].color, label="Computer Science", alpha=param_alpha, edgecolor='black', linewidth=3),
               Patch(facecolor=network_dict["Physics"].color, label="Physics", alpha=param_alpha, edgecolor='black', linewidth=3)]

    labels = [network_dict["Biology"].name, network_dict["Computer Science"].name, network_dict["Physics"].name]
...
def _rank_move_box_region(network, ax):

    rms = [[], [], [], [], []]
    regions = [NS('region', area_seoul, 'in', label="Seoul"),
               NS('region', area_capital, 'in', label="Capital\nArea"),
               NS('region', area_metro, 'in', label="Metropolitan\nCities"),
               NS('region', area_others, 'in', label="Others"),
               NS('name', inst_ists, 'in', label="-IST")]
    
    labels = [ns.label for ns in regions]
    ...
    def find_hit(name):
        ...
        for i in range(len(regions)):
            ...
            if ns.hit(network.net.nodes[name_norm]):
                return 4 - i
    ...
    for move in list(network.net.edges):
        ...
        if (r_move is not None and i is not None):

            rms[i].append(r_move / rank_length)

    bplot = ax.boxplot(rms, patch_artist=True, vert=True, labels=labels)
